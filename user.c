...
unsigned short long ReadRFCode(unsigned char cell)
{
    ...
    unsigned int address = 0x7f0;
    unsigned int taddress = 0x7f0;
    address = address + cell * 3;
    for (count=0; count++; count < 3) 
    {
    taddress = address;    
    PMADRL = taddress & 0x00FF;
	taddress >>= 8;
	PMADRH = taddress & 0x00FF;
	PMCON1bits.CFGS = 0;
	PMCON1bits.RD = 1;
	Nop();
	Nop();
...
unsigned char CommandGetLat(void)
{
	...
	data |= GPIO5_LAT;
	data <<= 1;
	data |= GPIO4_LAT; // Note: GPIO4 can't be output!
	data <<= 1;
	data |= GPIO3_LAT;
	data <<= 1;
	data |= GPIO2_LAT;
